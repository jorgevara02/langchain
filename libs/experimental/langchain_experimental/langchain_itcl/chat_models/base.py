"""OpenAI chat wrapper for ITCL purposes."""
...
class BaseChatITCL(BaseChatModel):
    ...
    itcl_api_key: Optional[SecretStr] = Field(default=None, alias="api_key")
    """Automatically inferred from env var `ITCL_API_KEY` if not provided."""
    itcl_api_base: Optional[str] = Field(default="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiU2NpZW5jZV93YXRjaGRvZyIsImV4cCI6NDg3MjIxNjY0Mn0.9G0ooi2SIy6FnuRc94VvhYmRahr3weY1S2IKlOC5ZSw", alias="base_url")
    """Base URL path for API requests, leave blank if not using a proxy or service 
        emulator."""
    openai_organization: Optional[str] = Field(default=None, alias="organization")
    """Automatically inferred from env var `OPENAI_ORG_ID` if not provided."""
    # to support explicit proxy for OpenAI
    itcl_proxy: Optional[str] = None
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["itcl_api_key"] = convert_to_secret_str(
            get_from_dict_or_env(values, "itcl_api_key", "ITCL_API_KEY")
        )
        ...
        values["itcl_api_base"] = values["itcl_api_base"] or os.getenv(
            "ITCL_API_BASE"
        )
        values["itcl_proxy"] = get_from_dict_or_env(
            values, "itcl_proxy", "ITCL_PROXY", default=""
        )

        client_params = {
            "api_key": (
                values["itcl_api_key"].get_secret_value()
                if values["itcl_api_key"]
                else None
            ),
            "organization": values["openai_organization"],
            "base_url": values["itcl_api_base"],
        ...
        itcl_proxy = values["itcl_proxy"]
        if not values.get("client"):
            if itcl_proxy and not values["http_client"]:
                try:
                    import httpx
                ...
                values["http_client"] = httpx.Client(proxy=itcl_proxy)
            sync_specific = {"http_client": values["http_client"]}
            values["client"] = openai.OpenAI(
                **client_params, **sync_specific
            ).chat.completions
        if not values.get("async_client"):
            if itcl_proxy and not values["http_async_client"]:
                try:
                    import httpx
                except ImportError as e:
                    raise ImportError(
                ...
                values["http_async_client"] = httpx.AsyncClient(proxy=itcl_proxy)
        ...
        **kwargs: Any,
        ...
        Example: Function-calling, Pydantic schema (method="function_calling", include_raw=False):
            .. code-block:: python
                ...
                from langchain_itcl import ChatITCL
                ...
                llm = ChatITCL(model="gpt-3.5-turbo-0125", temperature=0)
...
class ChatITCL(BaseChatITCL):
    """ITCL intern chat integration.

    Setup:
        Install ``langchain-itcl`` and set environment variable ``ITCL_API_KEY``.

        .. code-block:: bash

            pip install -U langchain-itcl
            export ITCL_API_KEY="your-api-key"
    ...
    Key init args — client params:
        ...
        api_key: Optional[str]
            ITCL API key. If not passed in will be read from env var ITCL_API_KEY.
    ...
    **NOTE**: Any param which is not explicitly supported will be passed directly to the
    ...
    invoked. For example:
        .. code-block:: python
            ...
            ChatITCL(..., frequency_penalty=0.2).invoke(...)
            ...
            ChatITCL(...).invoke(..., frequency_penalty=0.2)
    ...
    Tool calling:
        ...
        See ``ChatITCL.bind_tools()`` method for more.
    ...
    Structured output:
        ...
        See ``ChatITCL.with_structured_output()`` for more.
    ...
    Token usage:
        ...
        useful when incorporating ``ChatITCL`` into LCEL chains-- or when using
        methods like ``.with_structured_output``, which generate chains under the
        hood.

        .. code-block:: python

            llm = ChatITCL(model="gpt-4o", stream_usage=True)
            structured_llm = llm.with_structured_output(...)
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {"itcl_api_key": "ITCL_API_KEY"}
    ...
    def lc_attributes(self) -> Dict[str, Any]:
        ...
        if self.itcl_api_base:
            attributes["itcl_api_base"] = self.itcl_api_base

        if self.itcl_proxy:
            attributes["itcl_proxy"] = self.itcl_proxy
    ...
    def _stream(
        ...
        # To support users proxying Azure through ChatITCL, here we only specify
